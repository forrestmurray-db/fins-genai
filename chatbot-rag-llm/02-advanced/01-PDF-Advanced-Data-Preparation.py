...
# MAGIC %pip install transformers==4.35.2 "unstructured[pdf,docx]==0.10.30" langchain==0.0.348 llama-index==0.9.13 databricks-vectorsearch==0.22 pydantic==1.10.9 mlflow==2.9.0
...
# MAGIC
# MAGIC Using the <a href="https://unstructured.io/">`Unstructured`</a> library within a Spark UDF makes it easy to extract text. 
# MAGIC
...
# MAGIC
# MAGIC We'll extract the content and then use llama_index <a href="https://docs.llamaindex.ai/en/stable/api/llama_index.node_parser.SentenceSplitter.html">`SentenceSplitter`</a>, and ensure that each chunk isn't bigger than 500 tokens. 
# MAGIC
# MAGIC
...
def extract_doc_text(x : bytes) -> str:
  ...
  def clean_section(txt):
    # return clean_extra_whitespace(txt)
...
with requests.get('https://github.com/databricks-demos/dbdemos-dataset/blob/main/llm/databricks-pdf-documentation/Databricks-Customer-360-ebook-Final.pdf?raw=true') as pdf:
  doc2 = extract_doc_text(pdf.content)  
  print(doc2)
...
t = spark.read.table("pdf_raw")

# COMMAND ----------

pdf_test = (spark.table("pdf_raw")
              .limit(1)
...
indexes = vsc.list_indexes(VECTOR_SEARCH_ENDPOINT_NAME).get("vector_indexes", list())
indexes
